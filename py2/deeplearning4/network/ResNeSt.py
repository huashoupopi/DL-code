...
class ConvBNLayer(nn.Layer):
    def __init__(self,num_channels,num_filters,filter_size,stride=1,dilation=1,groups=1,act=None,name=None):
...
class SplatConv(nn.Layer):
    def __init__(self,
                 in_channels,channels,kernel_size,stride=1,padding=0,dilation=1,groups=1,bias=True,
                 radix=2,
                 reduction_factor=4,
                 rectify_avg=False,
                 name=None):
        ...
        # to calc gap
        self.conv2 = ConvBNLayer(num_channels=channels,num_filters=inter_channels,filter_size=1,stride=1,
            groups=groups,
            act="relu",
...
class BottleneckBlock(nn.Layer):
    def __init__(self,
                 inplanes,planes,stride=1,radix=1,cardinality=1,bottleneck_width=64,
                 avd=False,
                 avd_first=False,
                 dilation=1,
                 is_first=False,
...
class ResNeStLayer(nn.Layer):
    def __init__(self,inplanes,planes,blocks,radix,cardinality,bottleneck_width,
                 avg_down,
                 avd,
                 avd_first,
                 rectify_avg,
...
class ResNeSt(nn.Layer):
    def __init__(self,
        ...
        :param layers: 残差块数量
        :param radix: 基数超参数
        :param groups:组数  cardinality
        :param bottleneck_width:  残差块中瓶颈层的宽度
        :param dilated:  是否使用空洞卷积
        :param dilation:   空洞因子
        :param deep_stem:  是否使用深度初始卷积
        :param stem_width:   初始卷积层的输出通道数
        :param avg_down:   是否在下采样路径中使用平均池化
        :param rectify_avg:  是否使用修正的平均池化
        :param avd:        是否使用平均值下采样
        :param avd_first:  是否在第一个卷积之前使用平均池化
        :param final_drop:  最终分类之前的 丢弃率
        :param last_gamma:  示是否在最后一个批归一化层中添加可学习的缩放参数。
        :param class_num: 类别数
        """
...
if __name__ == "__main__":
    model = ResNeSt(layers=[3,4,23,3],radix=2,groups=1,bottleneck_width=64,deep_stem=True,stem_width=64,avg_down=True,avd=True,avd_first=True,class_num=102)
